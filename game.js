/* ──────────────────────────────────────────────────────────────────────────────
   PULSE — style.css  (pixel heat edition)
────────────────────────────────────────────────────────────────────────────── */
...
#hud-score {
  ...
  color: var(--white);
  /* pixel hard-shadow in ember colour */
  text-shadow:
    3px 3px 0 var(--ember),
...
#hud-combo {
  ...
  margin-top: clamp(2.8rem, 9vw, 5rem);
...
/* ── PIXEL HEART LIVES — top-left, horizontal ──
   FIX: container gap is 26px so box-shadows (21px wide) never overlap.
   Each .life-heart element is 3×3px; box-shadow draws the full 21×18px art.
────────────────────────────────────────────────── */
#lives-hud {
  position: fixed;
  top: 24px;
  ...
  display: flex;
  flex-direction: row;
  gap: 26px;          /* was 8px — increased to prevent overlap */
  z-index: 10;
  pointer-events: none;
  align-items: center;
}
...
/*
  16×14 pixel-heart (3px unit, box-shadow art):
  Same pixel map as before; only spacing fix is in the container.
*/
.life-heart {
...
/* ── COMPACT LEVEL PATH MAP — horizontal strip, bottom-center ──
   7 pixel-square nodes connected by thin lines.
   Width: 7×8px nodes + 6×14px connectors = 140px total.
   Active node enlarges to 8×8 and shows a level-name label below.
────────────────────────────────────────────────────────────────── */
#level-map {
  position: fixed;
  bottom: 62px;                   /* sits above the speed-pip bar */
  left: 50%;
  transform: translateX(-50%);
  z-index: 10;
  pointer-events: none;
  transition: opacity 0.4s ease;
  padding-bottom: 18px;           /* room for the label below active node */
}
...
/* Fixed-width wrapper keeps connectors aligned regardless of dot size */
.lm-node-wrap {
  display: flex;
...
/* Active level: dot grows */
.lm-node-wrap.lm-current .lm-dot {
  width: 8px;
  height: 8px;
}

/* Level name — floats below active dot only */
.lm-nodelabel {
  position: absolute;
  top: 13px;
  left: 50%;
  ...
  white-space: nowrap;
  color: transparent;             /* hidden by default; JS sets accent colour */
  line-height: 1;
  pointer-events: none;
...
/* Progress fill inside connector */
.lm-conn-fill {
  position: absolute;
  top: 0; left: 0;
  width: 0%;
  height: 100%;
  transition: width 0.4s steps(8), background 0.4s ease, box-shadow 0.3s ease;
}
...
.game-title {
  ...
  color: var(--white);
  /* Layered pixel shadow for depth */
  text-shadow:
    4px 4px 0 var(--ember),
...
/* Animated ring demo on start screen */
.ring-demo {
  width: 90px; height: 90px;
  border-radius: 50%;
  border: 2px solid rgba(255,140,0,0.30);
  box-shadow: 0 0 12px rgba(255,140,0,0.10), inset 0 0 12px rgba(255,140,0,0.04);
...
.ring-demo::before {
  ...
  animation: demoRing 2s ease-in-out infinite;
...
@keyframes demoPulse {
  0%,100% { box-shadow: 0 0 12px rgba(255,140,0,0.10), inset 0 0 12px rgba(255,140,0,0.04); }
  50%     { box-shadow: 0 0 26px rgba(255,140,0,0.28), inset 0 0 12px rgba(255,140,0,0.08); }
}
@keyframes demoRing {
  0%   { inset: -50px; opacity: 0.8; border-color: rgba(255,140,0,0.6); }
  60%  { inset: -22px; opacity: 1;   border-color: rgba(255,220,0,0.9); }
  80%  { inset: -22px; opacity: 1; }
  100% { inset: -50px; opacity: 0;   border-color: rgba(255,140,0,0.1); }
}
...
.go-level {
  ...
  color: var(--accent);
  text-shadow: 3px 3px 0 rgba(255,140,0,0.4), 0 0 18px var(--accent);
}
.new-hs {
  font-size: 0.48rem;
...
.btn {
  ...
  /* Hard pixel shadow — no border-radius */
  border-radius: 0;
  box-shadow:
    4px 4px 0 var(--ember),
    8px 8px 0 rgba(255,69,0,0.35);
